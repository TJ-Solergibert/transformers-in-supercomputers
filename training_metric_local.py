...
def training_function(args):
    ...
    path_to_model = os.path.join(os.path.join(os.path.dirname(os.path.realpath(__file__)), "distilbert-base-uncased"))
    path_to_dataset = os.path.join(os.path.join(os.path.dirname(os.path.realpath(__file__)), "emotion"))
    ...
    metric = evaluate.load("accuracy", process_id = int(os.environ["RANK"]), num_process= int(os.environ["WORLD_SIZE"]), experiment_id="slurm")
    ########################################################
    ...
    ####################### Training #######################
    ft0 = time.time()
    for epoch in tqdm(range(starting_epoch, num_epochs), desc = "Training...", unit = "epoch"):
        model.train()
        if args.with_tracking:
            total_loss = 0
